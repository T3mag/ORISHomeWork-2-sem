...
public class PutEditProfileCommandHandler : IRequestHandler<PutEditProfileCommand, bool>{
    ...
    public async Task<bool> Handle(PutEditProfileCommand request, CancellationToken cancellationToken) {
        ...
        var userFromDb = await _dbContext.Users.Include(x => x.UserInfo).ThenInclude(y => y.Country).FirstOrDefaultAsync(x => x.Id == _userContext.CurrentUserId!.Value, cancellationToken) ?? throw new ApplicationException("Пользователь не найден");
        var country = await _dbContext.Countries.FirstOrDefaultAsync(x => x.Id == request.Country, cancellationToken) ?? throw new ApplicationException("Не найдена страна");
        if (userFromDb.UserInfo is null) throw new ArgumentNullException(nameof(userFromDb.UserInfo)); 
