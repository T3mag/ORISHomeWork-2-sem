...
public class GetAllGamesQueryHandler : IRequestHandler<GetAllGamesQuery, GetAllGamesResponse> {
    private readonly IDbContext _dbContext;
    public GetAllGamesQueryHandler(IDbContext dbContext) => _dbContext = dbContext;
    public async Task<GetAllGamesResponse> Handle( GetAllGamesQuery request, CancellationToken cancellationToken){
        ...
        var result = await allGames.Select(x => new GetAllGamesResponseItem {
                Id = x.Id,
                Name = x.Name,
                ShortDescription = x.ShortDescription,
                ImageUrl = x.MediaFiles.Select(y => y.Path).ToList(),
                Rating = x.Rating,
                MainImage = x.MainImage!.Path,
                LastImages = x.MediaFiles.Select(y => y.Path).ToList(),
                Price = x.Price,
                Category = x.Categories.Select(y => y.Name).ToList(),
